...
- [1. 准备运行云崽](#1-准备运行云崽)
  ...
  - [1.10. 下载云崽插件（可选）](#110-下载云崽插件可选)
    ...
    - [1.10.2. 下载 PY 插件](#1102-下载-py-插件)
      ...
      - [1.10.2.2. 下载 PY 插件源码](#11022-下载-py-插件源码)
      - [1.10.2.3. 安装 PY 插件依赖](#11023-安装-py-插件依赖)
...
2. 解压、编译源码并安装
   ...
   sudo ./configure --enable-gpl --enable-version3 --enable-nonfree --disable-ffplay \
      --disable-ffprobe --enable-libopencore-amrnb --enable-libopencore-amrwb
   ```
   ...
   > - `--disable-ffplay --disable-ffprobe` 即关闭 FFplay 及 FFprobe 功能。因我们只需要语音编码功能，此处可关闭。
...
2. 安装 [Poetry](https://python-poetry.org "Poetry - Python denpendency management and packaging made easy")

   > **注意**
   >
   > ~~Alpine 请通过 `sudo apk add poetry` 安装。~~

   1. 使用 Python 安装 Poetry
